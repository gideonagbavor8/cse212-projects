...
public static class SetsAndMaps
{
    ...
    public static string[] FindPairs(string[] words)
    {
        ...
        {
            // Skip if both letters are the same
            if (word[0] == word[1])
            continue;

            var reversed = new string(new[] { word[1], word[0] });

            // If the reverse exists, record the pair
            if (wordSet.Contains(reversed))
            {
                // Create a canonical pair string to avoid duplicates
                var pair = string.Compare(word, reversed) < 0 ? $"{word} & {reversed}"
                    : $"{reversed} & {word}";
                pairs.Add(pair);
            }
    ...
    public static bool IsAnagram(string word1, string word2)
    {
        ...
        // If dictionary is empty, it's a perfect match
